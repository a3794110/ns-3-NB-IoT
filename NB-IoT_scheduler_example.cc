    ...
    */
    ...
    static uint16_t numberOfNodes = 4;
    double simTime = 5;//default is 0.1
    double interPacketInterval = 10;
    string scheduler = "ns3::PfFfMacScheduler";

    // for nb-iot
    // set scheduler simulation scenario
    // ch: channel option, 0 for NPUSCH, 1 for NPDSCH
    ...
    cmd.AddValue("interPacketInterval", "Packet Interval", interPacketInterval);
    cmd.AddValue("ch", "Channel option", ch);
    cmd.AddValue("R", "DCI repetition", R);
    cmd.AddValue("alpha", "Offset option", alpha);
    cmd.Parse(argc, argv);
    ...
    Ptr<LteEnbMac> enbMac = enbLteDevs.Get(0)->GetObject<LteEnbNetDevice>()->GetMac();
    enbMac->SetChannel(ch);
    enbMac->SetRepetition(R);
    enbMac->SetAlpha(alpha);
